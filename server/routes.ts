...
const searchParamsSchema = z.object({
  ...
  country: z.array(z.string().length(2)).default([]), // Changed to array of strings
  ...
  app.get("/api/ads", async (req, res) => {
    try {
      ...
      const response = await fetch(
        `https://graph.facebook.com/${FB_API_VERSION}/ads_archive?` +
          new URLSearchParams({
            ...
            ad_type,
            ad_reached_countries: JSON.stringify(country.includes("ALL") ? ["ALL"] : country), //Modified line
            limit: "24",
            fields,
      ...
      await storage.createSearchHistory({
        ...
        adType: ad_type,
        countries: country, //Modified line
      });
